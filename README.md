...
- <b>关于文字 :</b> 我这边用的方案中字体大小也是使用rem，也就意味着我们的段落文字随着屏幕增大，等比例缩放，这样一行所展示的文字个数也一样。这只是其中的一个方案，还有一种文字不使用rem作为单位，这样就需要针对字体做特殊的处理，使用一些预处理或者后处理的方式，针对dpr不同计算出不同dpr下文字对应的大小，这种方案[可参考](http://www.html-js.com/article/Mobile-terminal-H5-mobile-terminal-HD-multi-screen-adaptation-scheme%203041)
中有说明;
## 总结
哈哈,终于将这篇文章写完了,总结一哈😄
